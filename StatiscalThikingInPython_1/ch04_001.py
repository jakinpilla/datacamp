...
belmont.info()
time_series = belmont.Time.str[0:1].astype(int)*60 + belmont.Time.str[2:4].astype(int) +belmont.Time.str[5:7].astype(int)/100

belmont['time'] = time_series
belmont.info()
plt.boxplot(np.array(time_series))
plt.show()
...
len(removeOutliers(time_series, 1.5))

belmont_no_outliers = removeOutliers(time_series, 1.5)
